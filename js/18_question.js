// 1. User 클래스 만들기 아이디값(1,2,3...),이름,나이,자기소개 매서드(인스턴스)
class User {
...
//2. user01, user02, user03을 만들고 user01의 이름과 나이를 비구조화 할당하기
let user01 = new User(1, "seok", 30);
let user02 =new User(2, "minseok", 30);
let user03 = newUser(3,"jin",29);
...
let userArr= userArr.map((user) => user.age);
console.log(ageArr);
...
//5.id가 1인 유저의 나이를 바꾸기
// userArr=userArr.map((user)=>{
//   if(user.id ===1){
//     return {id: user.id, name: user.name, age:20}
//   } else {
//     return user;
//   }
// })
...
//6. 3초 뒤에 "데이터"라는 값 resolve하는 Promise를 반환하는 함수 만들기
function getData() {
